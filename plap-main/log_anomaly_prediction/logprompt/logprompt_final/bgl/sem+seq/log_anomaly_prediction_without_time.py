...
# 读取和预处理数据
def load_and_process_data(log_file_path, window_duration, slide_interval,
                          next_window_duration, interval, n_rows):
    # 读取指定数量的日志
    df_log = pd.read_csv(log_file_path, usecols=['Label', 'Timestamp', 'EventId', 'EventTemplate'], nrows=n_rows)
    ...
    while start_time_seconds < df_log['Timestamp'].max() and len(texts) < 10000:
        # 当前窗口的结束时间
...
# HDFS
log_file_path = '/home/zhouzt/experiment/log_anomaly_prediction/logparser/Drain/demo_result/BGL.log_structured.csv'
texts, labels = load_and_process_data(log_file_path, window_duration=2,
                                      slide_interval=2, next_window_duration=2, interval=0, n_rows=500000)
...
class LogDataset(Dataset):
    def __init__(self, texts, labels, max_length=512):
        ...
        # 读取嵌入文件并解析
        self.embeddings_df = pd.read_csv("/home/zhouzt/experiment/log_anomaly_prediction/logprompt/logprompt_final/bgl_id_template_embeddings.csv")
    ...
    def __getitem__(self, idx):
        ...
        for idx, eid in enumerate(event_ids):
            ...
            # 最终融合
            combined_embedding = torch.cat((final_semantic_embedding, final_sequential_embedding),
                                           dim=-1)
...
class LSTMWithAttention(nn.Module):
    def __init__(self, input_size=2 * 600, hidden_size=128, num_layers=1, num_labels=2):
...
# 创建模型并移动到 GPU
model = LSTMWithAttention(input_size=2 * 600, hidden_size=128, num_layers=1, num_labels=2).to(device)
...
def train_model(model, criterion, optimizer, train_loader, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for embeddings, labels in train_loader:
            ...
            # print("Device of embeddings:", embeddings.device)  # 打印 embeddings 的设备
            # print("Device of labels:", labels.device)  # 打印 labels 的设备
            optimizer.zero_grad()
...
def evaluate_model(model, test_loader):
    ...
    if os.path.exists('best_model.pth'):
        os.remove('best_model.pth')  # 删除文件
        print("Model file 'best_model1.pth' has been deleted.")
