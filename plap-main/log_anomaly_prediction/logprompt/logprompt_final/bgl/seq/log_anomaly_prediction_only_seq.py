...
class LogDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for idx, eid in enumerate(event_ids):
            ...
            # 最终融合
            # combined_embedding = torch.cat((final_semantic_embedding),dim=-1)
            final_embeddings.append(final_sequential_embedding)
...
class LSTMWithAttention(nn.Module):
    def __init__(self, input_size=600, hidden_size=128, num_layers=1, num_labels=2):
...
# 创建模型并移动到 GPU
model = LSTMWithAttention(input_size=600, hidden_size=128, num_layers=1, num_labels=2).to(device)
