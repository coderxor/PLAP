...
from avg_android import Dtrain
from avg_virtual_android import updated_model, updated_tokenizer, add_new_tokens
...
class LogDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        template_tokens = re.split(r'[ ,?_=]+', template)
...
# 读取文件到 DataFrame
train_file_path = 'D:/LogPPT/logppt-me/logs/Android/Android_2k.log_structured.csv'
train_df = pd.read_csv(train_file_path)

# 假设文件有 'Original Log' 和 'Template' 两列
sampled_df = train_df[['Content', 'EventTemplate', 'LineId', 'EventId']].sample(n=2000, random_state=42)
logs = list(zip(sampled_df['Content'], sampled_df['EventTemplate'], sampled_df['LineId'], sampled_df['EventId']))
...
def eval_model(model, dataloader, tokenizer):
    ...
    def postprocess_prediction(decoded_tokens, log_length):
        processed_tokens = []
        previous_is_wildcard = False
        for token in decoded_tokens:
            if token == "PARAM":
                if not previous_is_wildcard:
                    processed_tokens.append("<*>")
                    previous_is_wildcard = True
            else:
    ...
    with torch.no_grad():  # 禁用梯度计算
        for batch in dataloader:
            logs, input_ids, target_ids, templates, line_ids, event_ids = batch  # 假设 `line_ids` 存储的是日志的 LineId
            ...
            for original_log, prediction, template, line_id in zip(logs, predictions, templates, line_ids):
                ...
                log_length = len(re.split(r'[ ,?_=]+', template))  # 获取原始日志的长度
                # 替换 PARAM 并合并连续的 <*>
