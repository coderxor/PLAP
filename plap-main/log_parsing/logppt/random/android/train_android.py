...
from logppt.random.android.random_android import Dtrain
from virtual_android import updated_model, updated_tokenizer, add_new_tokens
...
class LogDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 动态扩充词汇表
        vocab = set(tokenizer.get_vocab())  # 将词汇表转换为 set 集合，查找速度更快
        new_tokens = [token for token in input_tokens if token not in vocab]
        if new_tokens:
...
# 读取文件到 DataFrame
train_file_path = '/logs/Thunderbird/Android_2k.log_structured.csv'
