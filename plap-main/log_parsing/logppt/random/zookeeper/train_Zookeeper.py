...
from logppt.random.zookeeper.random_Zookeeper import Dtrain
from virtual_Zookeeper import updated_model, updated_tokenizer, add_new_tokens
...
# 读取文件到 DataFrame
train_file_path = '/home/zhouzt/code/logs/Zookeeper/Zookeeper_2k.log_structured.csv'
...
def train_model(model, dataloader, optimizer, scheduler, tokenizer):
    model.train()  # 切换到训练模式
...
# 假设 total_steps 是训练过程中所有步骤的总数
total_steps = len(train_dataloader) * epochs  # 训练总步骤数 = 每个 epoch 的数据量 * 总的 epoch 数

# 设置 10% 的 warm-up 步数
warmup_steps = int(0.1 * total_steps)

# 创建学习率调度器（线性衰减，带有 warm-up）
scheduler = get_linear_schedule_with_warmup(
    optimizer,
    num_warmup_steps=warmup_steps,  # warm-up 步数
    num_training_steps=total_steps  # 总训练步骤数
)

for epoch in range(epochs):
    print(f"Epoch {epoch + 1}/{epochs}")
    avg_loss, accuracy = train_model(model, train_dataloader, optimizer, scheduler, tokenizer)
